// Ajout pour compatibilité CommonJS/ESM (__dirname)
...
app.set('trust proxy', 1); // Si tu utilises un proxy ou Vite
...
function requireAuth(req, res, next) {
   console.log('🧪 Middleware requireAuth - session =', req.session); // 👈 Ajoute ceci
...
// Appliquer à toutes les routes sensibles
app.use('/generate-report', requireAuth);
app.use('/last-report', requireAuth);
app.use('/report-pdf', requireAuth);
...
app.get('/report-pdf', async (req, res) => {
  ...
  const userReport = userReports.get(req.session.email);
...
app.get('/show-report/:token', (req, res) => {
  ...
  const filepath = path.join(REPORTS_DIR, `${token}.html`);
...
`;
    ...
    res.status(404).send('Rapport introuvable ou expiré.');
  }
});
