...
// Pour remplacer le lastReport global
const userReports = {};
...
app.get('/report-pdf', async (req, res) => {
  const report = req.session?.lastReport;
  if (!report || !report.html) return res.status(404).send('Aucun rapport');
...
app.post('/generate-report', async (req, res) => {
  ...
  (async () => {
    ...
    try {
      ...
      req.session.lastReport = { html, json: result, count: emails.length };
    } catch (err) {
...
app.get('/last-report', (req, res) => {
  const report = req.session?.lastReport;
  if (!report) return res.status(404).json({ error: "Aucun rapport généré" });
  res.json(report);
});
